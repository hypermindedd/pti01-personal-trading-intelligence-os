...
MUTATION_PATTERNS = {
    "mt5_order_send": re.compile(r"\b(?:mt5|MetaTrader5)\s*\.\s*order_send\s*\("),
    "trade_request_action": re.compile(r"\bTRADE_ACTION_(?:DEAL|PENDING|MODIFY|REMOVE|SLTP|CLOSE_BY)\b"),
    "chart_mutation_api": re.compile(r"\bObject(?:Create|Set|Delete|Move)\s*\("),
    "execution_adapter": re.compile(r"\b(?:ExecutionAdapter|BrokerMutationAdapter|DispatchPermit)\b"),
}
...
def scan_for_mutation_surface():
    ...
    for path in code_files():
        if path.suffix.lower() not in {".py", ".json", ".yaml", ".yml", ".mq5", ".mqh"}:
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        for name, pattern in MUTATION_PATTERNS.items():
            for match in pattern.finditer(text):
                line = text.count("\n", 0, match.start()) + 1
                hits.append({"path": str(path.relative_to(ROOT)), "line": line, "pattern": name})
    return hits
...
def main():
    ...
    failures = bool(missing or forbidden or mutation_hits or present_excluded_inputs)
    evidence = {
        "project": "PTI.01",
        "wave": "W0",
        ...
        "file_sha256": {
            name: digest(ROOT / name) for name in REQUIRED if (ROOT / name).is_file()
        },
    }
